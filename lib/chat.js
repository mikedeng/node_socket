...
emitUserList = function (io) {
	crud.read('users', { is_online: true },	{},
...
signIn = function (io, user_map, socket) {
	crud.update('users', { '_id': user_map._id, is_online: true }, function (result_map) {
		emitUserList(io);
...
signOut = function (io, user_id) {
	crud.udpate('users', {'_id': user_id}, {is_online: false}, function (result_list) {
		emitUserList(io);
	});
		...
		io.set('blacklist', [])
			...
			.on('connection', function (socket) {
				socket.on('adduser', function (user_map) {
					crud.read('users', {name: user_map.name}, {}, function (result_list) {
						var result_map,
							cid = user_map.cid
						;
						delete user_map.cid;
						if( result_list.length > 0 ){
							...
							user_map.is_online = true;
							crud.construct('users', user_map, function (result_list) {
								result_map = result_list[0];
								result_map.cid = cid;
				...
				socket.on('updatechat', function (chat_map) {
					if( chatterMap.hasOwnProperty( chat_map.dest_id )){
						chatterMap[ chat_map.dest_id ].emit('udpatechat', chat_map);
					}else {
						socket.emit('updatechat', { sender_id: chat_map.sender_id, msg_text: chat_map.dest_name + ' has gone offline' });
					}
				});
				...
				socket.on('updateavatar', function (avtr_map) {
					crud.update('users', {'_id': makeMongoId(avtr_map.person_id)},
						{ css_map: avtr_map.css_map }, function (result_list) {
							emitUserList(io);
						}
					);
				});
